                ...
                for (const line of lines) {
                    ...
                    if (block.links.length > 0) {
                        for (const link of block.links) {
                            ...
                            const pxEnd   = pxStart + this._textWidth(linkText, scale);

                            const underlineY = y + (8 * scale) - 2;
        ...
        for (let raw of lines) {
            ...
            while ((match = linkRegex.exec(line)) !== null) {
                links.push({
                    text: match[1],
                    url: match[2],
                    start: clean.indexOf(match[1]) // pixel mapping done later
                });
